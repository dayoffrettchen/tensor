...
class Qnetwork():
    def __init__(self, h_size, rnn_cell, myScope):
        ...
        self.scalarInput = tf.placeholder(shape=[None, 4900], dtype=tf.float32)
        self.imageIn = tf.reshape(self.scalarInput, shape=[-1, 70, 70])
...
class experience_buffer():
    ...
    def sample(self, batch_size, trace_length):
        ...
        i = batch_size
        while i > 0:
            sampled_episodes = random.sample(self.buffer, 1)[0]
            length = len(sampled_episodes) + 1 - trace_length
            if length < 1:
                continue
            i -=1
            point = np.random.randint(0, length)
            sampledTraces.append(sampled_episodes[point:point + trace_length])


        # sampled_episodes = random.sample(self.buffer, batch_size)
        # sampledTraces = []
        # for episode in sampled_episodes:
        #     length = len(episode) + 1 - trace_length
        #     if length < 1:
        #         continue
        #     print(length)
        #     point = np.random.randint(0, length)
