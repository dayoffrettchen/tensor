...
class SnakeEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.width = 70
        self.height = 70
        self.observation_space = np.zeros([self.width,self.height])
        self.viewer = None
    ...
    def step(self, action):
        ...
        self.observationfind()
        observation = self.observation_space
        info = None

        return observation, reward, done, info

    def observationfind(self):
        self.observation_space = np.zeros([self.width, self.height])
        for x, y in self.game.snake:
            self.observation_space[x][y] = -111
        x, y = self.game.dot
        self.observation_space[x][y] = 222

    # TODO: define observation
    def reset(self):

        start = (np.random.randint(self.width), np.random.randint(self.height))
        self.game = SnakeGame(self.width, self.height, start)
        self.observationfind()
        observation = self.observation_space
        return observation
